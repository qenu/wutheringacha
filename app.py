...
class MainWindow(QMainWindow):
    ...
    def populate_data(self):
        ...
        if self.tick_count % 18 == 0:
            ...
            self.center_text.setText(f"{spin(self.tick_count%10)} 正在整理數據")
        self.tick_count += 1
    ...
    def dropdown_update(self, selection: str = POOLTYPE[1]):
        ...
        dropdown.addItems([
            k 
            for k, v in self.backend.data.items() 
            if v and v.attempt
            ])
    ...
    def result_content(self, pool: str) -> None:
        ...
        if cum and pool == "角色活動":
            hit_miss = len([_ for _ in cum if _.name not in STANDARD_POOL])/len(cum)*100
